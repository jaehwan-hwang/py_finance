...
# 낙폭    -90% → 회복에 필요한 수익률   900.0%`,
    ...
    /* ── 7. 네 지표 ── */
    {
      ...
      blocks: [
        ...
        {
          t: "note",
          text: "<b>주의 — 위 표의 CAGR과 샤프지수는 서로 다른 방식으로 연율화한 값이다.</b> CAGR은 시작값과 끝값만 보고 기하평균으로 환산하고(<code>(끝/시작)**(1/년) - 1</code>), 샤프지수의 분자는 일간 수익률의 산술평균에 252를 곱한다(<code>returns.mean() * 252</code>). 두 값은 일치하지 않는다. 이 자료의 삼성전자는 CAGR 48.27%, 산술 연율화 49.44%다. 어느 쪽이 틀린 것이 아니라 <b>묻는 질문이 다르다.</b> 실제로 원금이 몇 배가 되었나는 CAGR, 하루하루의 평균적인 성과는 산술평균이다. 변동성이 클수록 둘의 차이가 벌어진다.",
        },
      ],
    },
